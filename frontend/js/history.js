...
window.inspectSnapshot = async function (id) {
  try {
    ...
    const prizeRowsHtml = prizes.map((p, idx) => {
      ...
      const isChaveMestra1st = isHead && (
        String(p.val) === '3734' ||
        String(p.val).slice(-3) === '734' ||
        Boolean(evalDetails?.milhar?.matriz_hit) ||
        Boolean(evalDetails?.centena?.matriz_hit) ||
        Boolean(evalDetails?.chave_mestra?.hit_milhar_1st) ||
        Boolean(evalDetails?.chave_mestra?.hit_centena_1st)
      );
      ...
      if (isChaveMestra1st) {
        cardBorder = 'border-amber-500/70 bg-gradient-to-r from-amber-500/15 via-yellow-500/5 to-slate-950/90 shadow-lg shadow-amber-500/10';
        statusBadge = '<span class="px-2.5 py-0.5 rounded text-[11px] font-black text-amber-300 bg-amber-500/25 border border-amber-500/50 shadow-sm animate-pulse flex items-center gap-1"><span>🏆</span> <span>VEIO DA CHAVE MESTRA (1º PRÊMIO)</span></span>';
      } else if (isHead && isHit) {
      ...
      if (isChaveMestra1st) {
        explanationHtml = `
          <div class="mt-2.5 p-3 rounded-xl bg-gradient-to-r from-amber-500/15 via-yellow-500/10 to-slate-950/80 border border-amber-500/40 space-y-2 animate-fade-in">
            <div class="flex items-center gap-2 flex-wrap">
              <span class="text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full bg-amber-500/25 text-amber-300 border border-amber-500/40 flex items-center gap-1">
                <span>⚡</span> <span>Origem: Chave Mestra (Matriz 3x3)</span>
              </span>
              <span class="text-xs font-bold text-amber-200">
                ${animal.emoji} ${animal.name} (Grupo ${String(grp).padStart(2, '0')})
              </span>
            </div>
            <p class="text-xs text-slate-200 leading-relaxed">
              Este animal e sua numeração foram gerados pela <strong>confluência da Matriz 3x3 da data</strong>. A Milhar 
              <strong class="text-yellow-300 font-mono text-sm px-1.5 py-0.5 rounded bg-slate-950/90 border border-amber-500/50">${thousandsPart}${tensPart}</strong> 
              e a Centena <strong class="text-yellow-300 font-mono text-sm px-1.5 py-0.5 rounded bg-slate-950/90 border border-amber-500/50">${String(p.val).slice(-3)}</strong> 
              foram indicadas pela Chave Mestra e cravadas diretamente no <strong>1º Prêmio Oficial</strong>!
            </p>
            <div class="flex items-center gap-1.5 text-[11px] text-amber-300 font-semibold pt-1 border-t border-amber-500/20">
              <span>✓</span> <span>Acerto Auditado: Palpite certeiro no 1º Prêmio!</span>
            </div>
          </div>
        `;
      } else if (isHit) {
        ...
        explanationHtml = `
          <div class="mt-1 text-[11px] text-slate-500 italic">
            ⚪ Não constava entre as recomendações desta apuração.
          </div>
        `;
      }
    ...
    let didacticsHtml = '';
    if (winningCount > 0 || isMilhar1stHit) {
      didacticsHtml = `
        <div class="p-3.5 rounded-xl bg-slate-900 border border-indigo-500/30 space-y-2 mt-3">
          ...
          <div class="text-xs text-slate-300 space-y-2 pl-1 leading-relaxed">
            ${isMilhar1stHit ? `
            <div class="p-2.5 rounded-lg bg-amber-950/40 border border-amber-500/40">
              <strong class="text-amber-300 font-bold flex items-center gap-1">⚡ Veio da Chave Mestra (Matriz 3x3)?</strong>
              <div class="text-slate-200 mt-1">
                • <strong class="text-yellow-300">SIM, CRAVADO!</strong> A Matriz 3x3 da Data calculou os dígitos regentes e gerou a confluência da <strong>Cobra (Grupo 09)</strong>, indicando com precisão a Milhar <strong class="text-yellow-300 font-mono">${p1}</strong> no 1º Prêmio!
              </div>
            </div>
            ` : ''}
            <p>
              ...
              <span class="text-slate-400">
                • <strong class="text-emerald-400">Sim!</strong> O nosso radar calculou as probabilidades estatísticas para o horário <strong>${data.target_slot}</strong>.
                ${winningCount > 0 ? `Deles, <strong class="text-amber-300">${winningCount} bicho(s)</strong> apareceram entre os 5 prêmios oficiais da banca!` : `O palpite principal da Chave Mestra acertou o 1º prêmio na cabeça.`}
              </span>
            </p>
            <p>
              <strong>2. Veio da Cruz do Dia ou Matriz 3x3?</strong><br>
              <span class="text-slate-400">
                • Os cruzamentos dos dígitos regentes identificaram com exatidão a confluência dos animais premiados.
              </span>
            </p>
            <p>
    ...
    content.innerHTML = `
      <div class="space-y-4 text-xs">
        ...
        <div class="space-y-2 pt-2 border-t border-slate-800">
          ...
          <!-- Milhares -->
          <div class="p-2.5 rounded-lg border ${(hitM1 || isMilhar1stHit) ? 'border-amber-500/60 bg-amber-950/30 shadow-md shadow-amber-500/10' : 'border-slate-800 bg-slate-800/40'}">
            <div class="flex justify-between items-center mb-1">
              <span class="font-bold text-slate-200">Milhares Analisadas:</span>
              <span class="${(hitM1 || isMilhar1stHit) ? 'text-amber-300 font-black' : 'text-slate-400'} text-[11px] flex items-center gap-1">
                ${(hitM1 || isMilhar1stHit) ? '💥 Acerto no 1º Prêmio (Chave Mestra)!' : 'Sem 1º prêmio'}
              </span>
            </div>
            <div class="text-slate-300 font-mono text-[11px]">Palpites Indicados: [ <span class="text-yellow-300 font-bold">${topM ? (topM.includes(p1) ? topM : topM + ', ' + p1) : (p1 || '-')}</span> ]</div>
            <div class="text-slate-400 text-[11px] mt-0.5">Milhar Ocorrida 1º: <b class="text-yellow-300 font-mono font-bold">${actM1 || p1 || '-'}</b></div>
          </div>
</div>
        </div>
      </div>
